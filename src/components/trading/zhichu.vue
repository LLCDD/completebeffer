<template>
  <div class="div1" style="width:100%">
    ...
    <div class="div6" v-if="bool">
      <button @click="previousPage()">上一页</button>
      <span>{{firstpage}}/{{lastpage}}</span>
      <button @click="nextPage()">下一页</button>
    </div>
    <div class="div6" v-if="!bool">没有更多数据了</div>
  </div>
</template>
<script>
export default {
  ...
  mounted() {
    ...
    const _this = this;
    _this.http
      .post("/api/center/wallet_log", { type: 2 })
      .then(res => {
        if (res.code == 200) {
  ...
  methods: {
    previousPage() {
      ...
      const _this = this;
      _this.http
        .post("/api/center/wallet_log", { page: _this.firstpage, type: 2 })
        .then(res => {
          _this.list = res.data.list.data;
        });
    },
