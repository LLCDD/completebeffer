    ...
    uploadphoto(e) {
      ...
      reader.onload = function(e) {
        ...
        _this.http
          ...
          .then(res => {
            if (res.code == 200) {
              _this.http.post("/api/center/wechat").then(res => {
                if (res.code == 200) {
                  console.log(res);
                  if (res.data.info.wechat_img == null) {
                    _this.name = _this.phone = _this.src = "";
                  } else {
                    _this.name = res.data.info["wechat_name"];
                    _this.phone = res.data.info.wechat_phone;
                    _this.src = res.data.info.wechat_img;
                  }
                  _this.$router.push("/zhifu/management");
                }
              });
              // --------------------------------
              _this.$toasted.success("绑定成功成功").goAway(1500);
